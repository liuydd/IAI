...
for pinyin in unigram:
    ...
    for i in range(len(unigram[pinyin]['words'])):
        ...
        poss = count / one_word_count[pinyin]
        cp[word] = poss
...
for pinyin2 in bigram:
    ...
    for i in range(len(word_list)):
        ...
        word_possibility[pinyin2][word_list[i]] = count_list[i] / two_word_count[pinyin2]
...
def vertebi(pinyin_list, char_list, stack):
    ...
    for char in char_list[str_len]:
        ...
        for key in stack:
            ...
            tmp_p = 0
            if flag:
                tmp_p = stack[key] * word_possibility[pinyins][chars] / char_possibility[pinyin_list[str_len - 1]][last_char]
            else:
                tmp_p = stack[key] * char_possibility[pinyin_list[str_len]][char]
            if str_len == (length - 1):
                tmp_p *= char_possibility[pinyin_list[str_len]][char]
            if tmp_p > p:
                p = tmp_p
                index = key
        if not index == '':
